...
export function runRemove(branch: string, opts: RemoveOptions): void {
  ...
  // Issue #1: check branch safety BEFORE touching the worktree. A branch that is
  // checked out in a worktree can't be deleted until the worktree is removed, so
  // if we removed first and only then discovered unmerged commits, we'd have
  // already destroyed the worktree while refusing to finish — leaving --force
  // nothing to act on. Refuse up front and keep everything intact instead.
  ...
  // Remove the worktree. Distinguish "path already gone" (safe to untrack) from
  // any other git refusal, e.g. uncommitted changes (issue #4).
  ...
  if (!rm.ok && !pathGone) {
    // git refused for a real reason (uncommitted changes, etc). Keep the entry
    // tracked and surface git's own message so the user can act on it.
  ...
  if (!opts.keepBranch) {
    ...
    if (!del.ok) {
      ...
      // Some other branch-delete problem (e.g. branch already gone): note it,
      // but the worktree removal still succeeded.
